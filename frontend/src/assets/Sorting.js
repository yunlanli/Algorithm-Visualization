...
`   Insertion Sort is an O(N^2) general-purpose sorting algorithm. In the best case where a sorted array is given, it sorts the array in linear time.
    ...
    "Bubble Sort": {
        text: 
`   Bubble Sort sorts the biggest element in the unsorted array each time, contrary to insertion sort. However, both algorithms have a worst case O(N^2) and best case O(N) time complexity.
...
`   Quick Sort is often the go-to algorithm for general-purpose sorting.
    ...
    "Merge Sort": {
        text: 
`   Merge Sort adopts the Divide-and-Conquer strategy. It guarantees a worst case O(NlogN) time complexity.
    ...
    "Heap Sort": {
        text: 
`   Heap Sort is an O(NlogN) general-purpose sorting algorithm. As the name suggests, it leverages the property of a binary heap.
    ...
    "Shell Sort": {
        text: 
`   Shell Sort, named after its inventor Donald Sell, was one of the first sorting algorithms to break the quadratic time bound.
